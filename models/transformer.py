...
class Transformer(nn.Module):
    def __init__(self, num_class,d_model=1024, layer_norm_eps= 1e-5, num_encoder_layers: int = 4,hidden_size=768):
        super(Transformer, self).__init__()
        encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=32, dim_feedforward=2048, dropout=0.1,
                                                activation=F.leaky_relu, layer_norm_eps=layer_norm_eps, batch_first=True, norm_first=False,
                                                )
        encoder_norm = nn.LayerNorm(d_model, eps=layer_norm_eps)
        self.layer_0 = nn.TransformerEncoder(encoder_layer, num_encoder_layers, encoder_norm)
        self.layer_1 = nn.TransformerEncoder(encoder_layer, 2, encoder_norm)
        # 从预训练模型中加载模型
        # self.model = torch.nn.Transformer(d_model=512,nhead=8)
        # 从预训练模型中加载配置
        # 获取合适的池化层
        # self.pooler = GeMText(2, d_model, d_model)
        self.cls_token_0 = nn.Parameter(torch.randn(1, 1, d_model))
        self.cls_token_1 = nn.Parameter(torch.randn(1, 1, d_model))


        self.fc = nn.Linear(d_model, num_class)

        # 初始化全连接层的权重
        for module in self.layer_0.modules():
            self._init_weights(module)

        for module in self.layer_1.modules():
            self._init_weights(module)
    ...
    def forward(self, input):
        ...
        b, l, _ = input.shape  # 单独先将batch缓存起来
        # cls_tokens = repeat(self.cls_token, '() n e -> b n e', b=b)
        # input = torch.cat([cls_tokens, input], dim=1)
        cls_token_1 = repeat(self.cls_token_1, '() n e -> b n e', b=b)
        segment_list = [cls_token_1]
        for i in range(0,l,512):
            cls_token_0 = repeat(self.cls_token_0, '() n e -> b n e', b=b)
            first_in = torch.cat([cls_token_0,input[:,i:i+512]], dim=1)
            first_out = self.layer_0(first_in)
            first_out = rearrange(first_out[:,0], 'b c -> b 1 c ')
            segment_list.append(first_out)



        first_layer_out = torch.cat(segment_list, dim=1)
        out = self.layer_1(first_layer_out)
