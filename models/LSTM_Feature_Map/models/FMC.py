...
class FMC(nn.Module):
    def __init__(self,class_num=6, inplanes=2048, stride=1):
        super(FMC, self).__init__()

        self.conv1 = nn.Conv2d(inplanes, 2048, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(2048)
        self.ca1 = ChannelAttention(2048)
        self.sa1 = SpatialAttention()

        self.conv2 = nn.Conv2d(inplanes, 2048, kernel_size=3, stride=stride, bias=False)
        self.bn2 = nn.BatchNorm2d(2048)
        self.ca2 = ChannelAttention(2048)
        self.sa2 = SpatialAttention()

        self.conv3 = nn.Conv2d(inplanes, 2048, kernel_size=3, stride=stride,  bias=False)
        self.bn3 = nn.BatchNorm2d(2048)
        self.ca3 = ChannelAttention(2048)
        self.sa3 = SpatialAttention()

        self.conv4 = nn.Conv2d(inplanes, 2048, kernel_size=3, stride=stride,  bias=False)
