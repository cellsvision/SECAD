...
class BiLSTM_Attention(nn.Module):
    ...
    def forward(self, input):
        ...
        hidden_state = Variable(torch.zeros(1*2, input.shape[1], self.hidden_dim)).to(input.device)# [num_layers(=1) * num_directions(=2), batch_size, n_hidden]
        cell_state = Variable(torch.zeros(1*2, input.shape[1], self.hidden_dim)).to(input.device)# [num_layers(=1) * num_directions(=2), batch_size, n_hidden]
        ...
        # age = age.reshape(-1,1)
        # attn_output = torch.cat([attn_output,gender],dim=1)
